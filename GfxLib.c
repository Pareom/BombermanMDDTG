...
/* Valeur de la callback de gestion du clavier relache */
...
void ecrisImageTransparent(int x, int y, int largeur, int hauteur, const unsigned char *donnees, int r, int v, int b)
{
	int *pixels = BVR2ARVBTransparent(largeur, hauteur, donnees, r, v, b);
	ecrisImageARVB(x, y, largeur, hauteur, pixels);
	free(pixels);
}
int *BVR2ARVBTransparent(int largeur, int hauteur, const unsigned char *donnees, int r, int v, int b)
{
	const unsigned char *ptrDonnees;
	unsigned char *pixels = (unsigned char*)malloc(largeur*hauteur*sizeof(int));
	unsigned char *ptrPixel;
														...
														pixels de l'image */
	{
		...
		ptrPixel[1] = ptrDonnees[1];
		ptrPixel[2] = ptrDonnees[2];
		if(ptrPixel[0] == r && ptrPixel[1] == v && ptrPixel[2] == b)//Code couleur Ignoré
		{
		ptrPixel[3] = 0x000; //Opacité NULL
		}
		else
		{
		ptrPixel[3] = 0x0FF;//Opacité Totale
		}
		ptrDonnees += 3; /* On passe a la premiere composante du pixel suivant */
		ptrPixel += 4; /* On passe au pixel suivant */
	}
